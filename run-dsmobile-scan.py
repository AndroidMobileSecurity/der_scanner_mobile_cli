...
class DSMobile:
    """
    Class for interact with DSMobile system through REST API
    """
    def __init__(self, base_url, token, file, profile, testcase):
        # self.report_path = 'DSMobile_scan_report.json'
    ...
    def get(self, url):
        """
        Get method for DSMobile REST API.
    ...
    def create_report(self, scan_result, type):
        ...
        if type == 'standard' or type == 'grouping':
            report_name = 'dsmobile_scan_{0}_report.json'.format(type)
        else:
            report_name = 'dsmobile_scan_report-testcase_{}.json'.format(self.testcase)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Start scan and get scan results from DSMobile')
    ...
    # Arguments for DSMobile
    parser.add_argument('--dsmobile_url', type=str, help='DSMobile url', required=True)
...
if __name__ == '__main__':
    ...
    dsmobile_url = arguments.dsmobile_url
    dsmobile_token = arguments.token
    dsmobile_profile = arguments.profile
    dsmobile_testcase_set = set(arguments.testcase)
    ...
    for dsmobile_testcase_id in dsmobile_testcase_set:

        if len(dsmobile_testcase_set) > 1:
            log.info('Processing testcase {0}'.format(dsmobile_testcase_id))
        else:
        ...
        dsmobile = DSMobile(dsmobile_url, dsmobile_token, apk_file, dsmobile_profile, dsmobile_testcase_id)

        log.info('Start automated scan with test case Id: {0}, profile Id: {1} and file: {2}'.format(
            dsmobile_testcase_id, dsmobile_profile, apk_file))

        scan_id = dsmobile.start_scan()
        ...
        while not scan_complete:
            ...
            scan_complete = dsmobile.get_scan_status(scan_id)
            time.sleep(30)

        log.info('Scan complete, trying to get scan result')
        scan_result = dsmobile.get_scan_result(scan_id)
        if not scan_result:
            continue

        log.info('Scan complete, analysing issues')
        short_stat = dsmobile.get_short_stat(scan_id)
        if not short_stat:
            sys.exit(5)
        log.info('Vulnerability details: {0}'.format(short_stat))

        for i in scan_result:
            i['id'] = str(dsmobile_testcase_id) + '-' + str(i['id'])#add testcase to id
            i['scan_id'] = scan_id

        if 'separate' in report_types:
            dsmobile.create_report(scan_result, type)
    ...
    for type in report_types:
        if type == 'standard' or type == 'grouping':
            ...
            dsmobile.create_report(common_result, type)
